...
本文件描述已确认的目标设计；Stage 1～4 已完成。Stage 4：Participation 于用户 2026-09-24 授权后实施并通过验收，具体交付与验证状态见第 10 节；Stage 5 仍等待后续实施指令。
...
Stage 4 实施细则（2026-09-24）：

- 学生通过 POST 领取自己正式成功报名的随机凭证，重复领取返回原码；凭证独立表按报名编号唯一，不改变 Stage 3 的发送与消费链路，旧成功报名无需回填即可领取。
- 用户确认核验窗口为 `[场次开始时间, 场次结束时间)`，无提前或延后窗口。核验须同时满足组织者角色、活动归属、对应场次与成功报名事实。
- 核验事务锁定对应报名行；参与表以报名编号作为主键，通过外键关联凭证与核验人。窗口内重复核验返回首次记录，窗口外拒绝，历史记录继续可查；requestId 的 SUCCESS 不变。
- 我的参与记录与所属场次到场名单分页展示已核验记录，不暴露凭证或手机号。凭证属于可出示的随机码，第一版不提供动态防转让、二维码生成或自动缺席认定。
...
- Stage 1 的 Git 提交由用户手动完成；用户已确认掌握 Stage 1，2026-09-12 授权并开始实施 Stage 2；2026-09-20 授权实施 Stage 3。2026-09-24 用户授权实施 Stage 4；Stage 5 收到后续指令后再推进。
...
- 验收前修正了新测试的导入冲突及 Bearer 认证头；Docker 中途停止造成一次环境失败，恢复后完成以上最终全量回归。应用上下文已关闭，临时测试容器已确认自动回收；未执行开发数据库补表、未修改开发业务数据、未执行 Git 命令。此条记录截至 Stage 3 验收时；Stage 4 后续状态见下文。
- Stage 4 已完成（2026-09-24）：成功报名凭证领取、组织者角色与活动归属校验、按场次起止时间核验、重复核验幂等、我的参与记录与到场名单分页；新增两张 MySQL 表及 [Stage 4 学习与复现指南](STAGE4_GUIDE.md)。既有报名成功记录可直接领取凭证；未改动 Lua、MQ 或报名终态流程。
- Stage 4 最终验收：2026-09-24 17:16（Asia/Shanghai）执行 `mvn -B -ntp verify -Pintegration`，17 项单元测试 + 10 项 Account 集成测试 + 21 项 Activity 集成测试 + 21 项 Registration 集成测试 + 13 项 Participation 集成测试，共 **82 项全部通过**，失败、错误、跳过均为 0，JAR 打包成功。
- 新增启动验收在随机 HTTP 端口和隔离 MySQL、Redis、RabbitMQ 环境完成：真实健康检查 UP，基于真实报名消费事务生成的 SUCCESS，通过 HTTP 领取凭证、组织者核验，再查询个人参与记录及到场名单；核验后报名 SUCCESS 与剩余容量保持不变。
- Stage 4 正确性验证：12 个并发领取请求返回同一码，12 个并发核验请求只生成一条参与记录；验证角色、报名归属、跨组织者、错误场次、无成功事实、开始含/结束不含、窗口外重复核验、分页隔离、主键/外键、插入后异常整笔回滚及重试。这些是正确性验证，不是吞吐量压测。
- 首轮沙箱内构建受到网络和 Docker 访问限制，取得执行权限后运行隔离测试；首轮完整测试有 1 项测试故障注入写法错误（MyBatis 接口不能使用 Mockito 的抽象真实方法调用），改为真实插入后在读取环节注入异常，最终完整回归以上述通过结果为准。应用上下文已关闭，临时测试容器已确认全部回收；没有补建开发数据库表、没有修改开发业务数据、没有执行 Git 命令。Stage 5 未实施。
